...
cover: https://global-image-bed-1302614822.cos.ap-nanjing.myqcloud.com/images202301072242526.png
---
> 上文：{% post_link 从0开始配置你的manjaro-i3wm的安装 %}

# 卸载i3bar

![image-20230108131943798](https://global-image-bed-1302614822.cos.ap-nanjing.myqcloud.com/images202301081319902.png)

这是manjaro自带的i3wm所携带的i3bar，个人感觉i3bar并不好看，我们会用polybar来代替它。我们通过以下命令打开i3wm的配置文件。
...
![image-20230108200843460](https://global-image-bed-1302614822.cos.ap-nanjing.myqcloud.com/images202301082008907.png)
...
切换到normal模式之后输入`:wq`保存并退出vim编辑器，按下`Mod`+`Shift`+`R`让i3wm重新加载配置文件。

> 直接让i3wm启动polybar其实并不是最优的解决方案，此方案会导致i3wm重载配置文件后出现多个polybar。
>
> 你可以写一个脚本文件存放在任意位置，其中的内容为：
>
> ```bash
> killall -e polybar  # 终止已有的polybar
> polybar				# 打开polybar
> ```
>
> 然后为该文件赋予执行权限`chmod +x <文件路径>`，并将`exec_always polybar`改为`exec_always <脚本文件的绝对路径>`。

然后在`.config/polybar`下新建文件`config`，在其中编写你的配置，这里放出一份我自用的
...
modules-right = xbacklight alsa network cpu temperature battery date
...
安装好的效果大概是这样的![image-20230108205621058](https://global-image-bed-1302614822.cos.ap-nanjing.myqcloud.com/images202301082056601.png)

github上还有一个叫做[polybar themes](https://github.com/adi1090x/polybar-themes)的项目，感兴趣的可以clone下来，执行`setup.sh`之后在`~/.config/polybar/`下找到`launch.sh`，执行后可以看到帮助，选择喜欢的主题即可（具体参见该项目的README）。
...
在`~/.i3/config`的任意位置新增这样一串代码

```bash
exec_always --no-startup-id feh --randomize --bg-fill <存放壁纸的目录>
```

`--bg-fill`是壁纸填充模式，具体可以在终端通过`man feh`查看。

# 终端模拟器的选择

市面上有很多终端模拟器，比如gnome-terminal、konsole、deepin-terminal等，可供选择的有很多，你可以挨个安装后尝试，在这里，我只推荐一个——alacritty。

Why？~~因为你实在是太美~~ 它的优点我已经列在下面了
...
桌面环境自带的terminal在窗口管理器里用起来总是碍手碍脚的，比如deepin-terminal会出现较大的圆角，gnome-terminal无法正常渲染帮助页面......

GPU渲染也是一个重要的加分项，GPU渲染可以加快你的终端展示大量文字时的速度，同时降低CPU使用率，下图是deepin-terminal和alacritty的成绩

```bash
tree /  6.58s user 7.18s system 81% cpu 13.698 total  # deepin-terminal
```

```bash
tree /  3.23s user 3.10s system 98% cpu 6.397 total   # alacritty
```

But，alacritty有一个致命的缺点——配置过于麻烦，当然，我会放出我目前用的配置文件供参考
...
key_bindings:
  ...
  - { key: NumpadEnter,             chars: "\n"                              }
```
替换方式非常简单，更改一下`$mod + Return`的快捷键就行了

![image-20230108212200292](https://global-image-bed-1302614822.cos.ap-nanjing.myqcloud.com/images202301082122135.png)

更换终端模拟器之后，之前那个丑兮兮的bitmap字体也会跟着换掉

# 文字渲染问题

文字渲染有问题几乎都是字体的锅，执行下面这行命令补全所有常用字体即可

```bash
sudo pacman -Sy wqy-zenhei wqy-microhei wqy-bitmapfont adobe-source-code-pro-fonts adobe-source-han-mono-cn-fonts adobe-source-han-sans-cn-fonts adobe-source-han-serif-cn-fonts
```
